...
final class ContentSecurityPolicyHandler
{
    ...
    public function __construct(private readonly NonceGenerator $nonceGenerator)
    {
    }
     ...
     */
    public function getNonces(RequestInterface $request, ResponseInterface $response): array
    {
        if ($request->hasHeader(self::SCRIPT_NONCE_HEADER) && $request->hasHeader(self::STYLE_NONCE_HEADER)) {
            return [
                'csp_script_nonce' => $request->getHeader(self::SCRIPT_NONCE_HEADER),
                'csp_style_nonce' => $request->getHeader(self::STYLE_NONCE_HEADER),
            ];
        }

        if ($response->hasHeader(self::SCRIPT_NONCE_HEADER) && $response->hasHeader(self::STYLE_NONCE_HEADER)) {
            return [
                'csp_script_nonce' => $response->getHeader(self::SCRIPT_NONCE_HEADER),
                'csp_style_nonce' => $response->getHeader(self::STYLE_NONCE_HEADER),
            ];
        }
