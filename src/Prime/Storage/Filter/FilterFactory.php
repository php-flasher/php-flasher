...
final class FilterFactory implements FilterFactoryInterface
{
    ...
    {
        $this->addCriteria('priority', static fn (mixed $criteria) => new PriorityCriteria($criteria));
        $this->addCriteria('hops', static fn (mixed $criteria) => new HopsCriteria($criteria));
        $this->addCriteria('delay', static fn (mixed $criteria) => new DelayCriteria($criteria));
        $this->addCriteria('order_by', static fn (mixed $criteria) => new OrderByCriteria($criteria));
        $this->addCriteria('limit', static fn (mixed $criteria) => new LimitCriteria($criteria));
        $this->addCriteria('stamps', static fn (mixed $criteria) => new StampsCriteria($criteria));
        $this->addCriteria('filter', static fn (mixed $criteria) => new FilterCriteria($criteria));
    }
